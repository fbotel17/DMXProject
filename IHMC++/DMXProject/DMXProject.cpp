...
void DMXProject::createFormForSelectedEquipements(const QList<QString>& selectedEquipements, const QString& selectedScene)
{
	...
	if (countCanaux > 0) {
		QMessageBox msgBox;
		msgBox.setText("La scene selectionnee possede deja une configuration. Voulez-vous la supprimer ?");
...
void DMXProject::fillSceneComboBox2() {
	...
	// Récupérer toutes les scènes et les ajouter au QComboBox
	Scene scene;
	QList<Scene> scenes = scene.getAllScenes();
	foreach(const Scene & s, scenes) {
...
void DMXProject::fillEquipCheckBoxes() {
	...
	if (layout != nullptr) {
		...
		delete layout; // Supprimer le layout existant
	}

	// Créer un nouveau layout pour checkBoxEquip
	layout = new QVBoxLayout(ui.checkBoxEquip);

	// Remplir avec les nouveaux QCheckBox
	QSqlQuery query("SELECT id, nom FROM equipement");
	if (query.exec()) {
		while (query.next()) {
			int id = query.value(0).toInt();
			QString name = query.value(1).toString();

			// Créer une nouvelle QCheckBox pour chaque équipement
			QCheckBox* checkBox = new QCheckBox(name, ui.checkBoxEquip);
			checkBox->setObjectName(QString::number(id)); // Utiliser l'ID de l'équipement comme nom de l'objet
			layout->addWidget(checkBox);
		}
	}
	else {
		qDebug() << "Erreur lors de l'exécution de la requête de sélection des équipements:" << query.lastError().text();
	}

	// Définir le layout à checkBoxEquip
	ui.checkBoxEquip->setLayout(layout);
	ui.checkBoxEquip->update(); // Forcer la mise à jour de l'interface utilisateur
	ui.checkBoxEquip->repaint(); // Forcer le repaint de l'interface utilisateur
	ui.checkBoxEquip->setVisible(true); // S'assurer qu'il est visible
}
...
void DMXProject::onValidSceneEquipButtonClickedArduino() {
	...
	if (sceneIndex != -1) {
		QString sceneName = ui.SceneComboBox->currentText();
		QList<int> selectedEquipIds;

		foreach(QObject * obj, ui.checkBoxEquip->children()) {
			QCheckBox* checkBox = qobject_cast<QCheckBox*>(obj);
			if (checkBox && checkBox->isChecked()) {
				selectedEquipIds.append(checkBox->objectName().toInt());
			}
		}
...
void DMXProject::saveSceneEquipmentData(int idScene) {
	...
	QMessageBox::information(nullptr, "Succès", "La scène a été enregistrée avec succès.");
...
void DMXProject::updateChampSliderValue(int value) {
	if (m_currentChampIndex >= 0 && m_currentChampIndex < m_champSliderValues.count()) {
		...
		QString message = equip + " " + champName + " : " + QString::number(value) + "\n";
...
void DMXProject::ajouterScene() {
	...
	addSceneDialog->setWindowTitle("Ajouter une nouvelle scene");

	// Créer un layout vertical pour la boîte de dialogue
	QVBoxLayout* dialogLayout = new QVBoxLayout(addSceneDialog);

	// Ajouter un QLabel pour afficher le texte d'instruction
	QLabel* instructionLabel = new QLabel("Entrez le nom de la nouvelle scene:");
	dialogLayout->addWidget(instructionLabel);
	...
	connect(buttonBox, &QDialogButtonBox::accepted, [=]() {
		...
		// Afficher un message de confirmation
		QMessageBox::information(this, "Scene ajoutee", "La nouvelle scene a ete ajoutee avec succes : " + newSceneName);
